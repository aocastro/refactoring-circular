...
import { type NPSResponse } from "@/data/admin";
...
const AdminNPSContent = () => {
  ...
  const [adminNpsStats, setAdminNpsStats] = useState<any>(null);
  const [adminNpsHistory, setAdminNpsHistory] = useState<any>(null);
  const [adminNpsResponses, setAdminNpsResponses] = useState<any>(null);
  useEffect(() => {
    const fetchData = async () => {
      try {
        const res_adminNpsStats = await api.get('/api/admin/nps-stats');
        setAdminNpsStats(res_adminNpsStats.data);
        const res_adminNpsHistory = await api.get('/api/admin/nps-history');
        setAdminNpsHistory(res_adminNpsHistory.data);
        const res_adminNpsResponses = await api.get('/api/admin/nps-responses');
        setAdminNpsResponses(res_adminNpsResponses.data);
      } catch (err) {
